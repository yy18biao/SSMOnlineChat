<template>
  <div class="reg-page">
    <div class="main">
      ...
      <div class="input">
        <el-input v-model="mobileForm.nickname" type="text" class="nickname-input" id="nickname-input" placeholder="请输入昵称"/>
...
import {regService, sendRegCodeService} from "@/apis/user/reg.js";
...
async function getRegCode() {
  ...
  await sendRegCodeService(mobileForm.phone, 1)
  txt.value = '59s'
  let num = 59
  timer = setInterval(() => {
    num--
...
// 注册
async function reg(){
  if (mobileForm.nickname === null || mobileForm.nickname === '')
    return ElMessage.error("请输入昵称")
  else if (mobileForm.phone === null || mobileForm.phone === '')
    return ElMessage.error("请输入手机号码")
  else if (!phoneRegex.test(mobileForm.phone))
    return ElMessage.error("手机号码格式错误")
